...
async def main():
    ...
    graph = ConnectionGraph(
        agent_ids,
        seed=GRAPH_SEED if GRAPH_SEED != 0 else None,
        min_degree=GRAPH_MIN_DEGREE,
        degree_exponent=GRAPH_DEGREE_EXPONENT,
        degree_factor=GRAPH_DEGREE_FACTOR,
        degree_offset=GRAPH_DEGREE_OFFSET,
        max_degree=(int(GRAPH_MAX_DEGREE) if GRAPH_MAX_DEGREE not in (None, "") else None),
    ).get_graph()
