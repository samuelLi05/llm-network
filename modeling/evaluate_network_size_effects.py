"""
    A script for evaluating how trends in model quality change with the size of the network
"""
...
if __name__ == '__main__':
    ...
    for network_size in ALL_SIZES:
        nsize_path = RUNS_DIR / network_size

        train_path = nsize_path / 'train'
        test_path = nsize_path / 'test'
        if not train_path.exists() or not test_path.exists():
            raise FileNotFoundError(f"Train or test directory missing for network size {network_size} at {nsize_path}")
        ...
        try:
            ...
            gamma_plots_dir = combo_dir / 'gamma_objective_plots' / network_size
            gamma_plots_dir.mkdir(parents=True, exist_ok=True)
            gamma_csv_dir = combo_dir / 'gamma_objective_data' / network_size
            ...
            for raw_model_name in rollout_maps_test.keys():
                ...
                per_run_df['llm'] = LLM
                per_run_df['topic'] = TOPIC
                per_run_df['network_size'] = network_size
            ...
            combo_csv = combo_dir / f'{LLM}__{TOPIC}__{network_size}_model_rankings.csv'
            ...
            params_csv = params_dir / f'{LLM}__{TOPIC}__{network_size}_fitted_params.csv'
