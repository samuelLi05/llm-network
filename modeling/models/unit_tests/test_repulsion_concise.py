...
def _sim_tanh_repulsion_no_homophily(Abar,
    ...
    if not lambda_self + lambda_social + lambda_init + lambda_bias + lambda_repulsion == 1.0:
        ...
        for _ in range(horizon):
            ...
            x = (lambda_self * x + lambda_social * Abar @ x \
                    + lambda_init * states[0] + \
                        lambda_bias * bias + \
                            lambda_repulsion * repulsion_term) + noise_std * rng.normal(size=n)
            # clip to [-1, 1]
            x = np.clip(x, -1.0, 1.0)
...
class TestBaseRepulisionRecovery(unittest.TestCase):
    CASES = [
